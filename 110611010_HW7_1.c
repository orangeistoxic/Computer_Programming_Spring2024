...
int main(){
    ...
    while(fgets(input,sizeof(input),stdin)){
        ...
        if(strcmp(command,"New")==0){
            ...
            (*newNode).integer = atoi(token);
            (*newNode).next = NULL;
            if(first==NULL){  // If the stack is empty, the first node is the new node.
                first = newNode;
                current = newNode;
            }
            else{  //Push current node to the next node.
                (*current).next = newNode;
                current = newNode;
            }
        }
        else if (strcmp(command, "Print all") == 0)
        {
            ...
            else{ // from the first node, find the last node with pointer NOT to NULL.
                temp = first;
